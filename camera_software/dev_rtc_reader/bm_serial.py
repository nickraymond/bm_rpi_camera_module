...
class BristlemouthSerial:
    ...
    def __init__(self, uart=None, node_id: int = 0xC0FFEEEEF0CACC1A) -> None:
        ...
        if uart is None:
            ...
            self.uart = serial.Serial(
                port="/dev/serial0",   # <<< use the stable alias
                baudrate=115200,
                ...
                dsrdtr=False,
                xonxoff=False,
                exclusive=True,        # <<< Linux-only; avoids double-opens
            )
            print(f"[UART-OPEN] {self.uart.name} open={self.uart.is_open}")
        else:
            self.uart = uart
    ...
    def _reopen_uart(self):
        try:
            if getattr(self, "uart", None) and self.uart.is_open:
                self.uart.close()
        except Exception:
            pass
        self.uart = serial.Serial(
        ...
        print(f"[UART-REOPEN] {self.uart.name} open={self.uart.is_open}")
        
    def _read_until_idle(self, timeout_s: float) -> bytes:
        end = time.monotonic() + timeout_s
        buf = bytearray()
        while time.monotonic() < end:
            try:
                waiting = self.uart.in_waiting  # may raise OSError(25) if FD is bad
            except OSError as e:
                print(f"[UART][ERR] in_waiting failed: {e}; reopening…")
                self._reopen_uart()
                # after reopen, continue the loop without extending timeout
                continue
            if waiting:
                chunk = self.uart.read(waiting)
                if chunk:
                    buf += chunk
                    # extend idle window after activity
                    end = time.monotonic() + timeout_s
            else:
                time.sleep(0.01)
        return bytes(buf)
        
    def lock_uart_and_write_bytes(self, bytes_: bytes) -> bool:
        try:
            self.uart.reset_output_buffer()
            n = self.uart.write(bytes_)
            self.uart.flush()
            return n == len(bytes_)
        except serial.SerialTimeoutException:
            print("[UART][ERR] write timed out")
            return False
        except Exception as e:
            print(f"[UART][ERR] write failed: {e}; reopening…")
            self._reopen_uart()
            return False

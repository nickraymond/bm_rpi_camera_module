...
# text-only handler for topic 'camera/capture/image'
# Supports: '1' (defaults) or CSV key=val (no spaces): res=1080p,burst=3,int=200ms
import os, sys, time
from pathlib import Path
...
from process_image import capture_image, IMAGE_DIRECTORY  # keep as 'process_image' for now
...
def handle(node_id, topic: str, data: bytes, ctx):
    ...
    if not CAMERA_LOCK.acquire(blocking=False):
        print("[CAM/IMG] busy, dropping trigger")
        return
    try:
        for i in range(max(1, burst)):

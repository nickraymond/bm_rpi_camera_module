...
from .status_util import ack_print # <<< obsolete now that we have spotter_log
    ...
    try:
        ...
        with CameraLock(timeout_s=8.0):
            for i in range(burst):
                ...
                spotter_log(ctx,
                            level="INFO", tag="CAM", msg="image saved",
                            file=_P(path).name, res=res, idx=i+1, burst=burst, size=size)
        ...
        spotter_log(ctx, level="ERR", tag="CAM", msg="image failed", reason="busy")
    except Exception as e:
        print(f"[CAM/IMG][ERR] {e!r}")
        spotter_log(ctx, level="ERR", tag="CAM", msg="image failed", reason=type(e).__name__)

...
def main(spotter_ids, start_date, end_date):
    for spotter_id in spotter_ids:
        print(f"\nProcessing Spotter ID: {spotter_id}")
        try:
            api_data = api_login(spotter_id, start_date=start_date, end_date=end_date)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', default="2025-05-17T00:00:00Z", help='Start date in ISO format')
    parser.add_argument('--end', default="2025-05-19T00:00:00Z", help='End date in ISO format')

    args = parser.parse_args()
    main(args.spotters, args.start, args.end)

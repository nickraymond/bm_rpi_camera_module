...
# spotter data not being saved to logs, want to get waves, baro, winds...
# not sure of this is pagenating correctly or not
# not processing JPG files correctly.
...
def process_json_0(json_data, img_directory_base):
...
def process_json(json_data, img_directory_base):
    """Groups data by bristlemouth_node_id and saves CSVs into node-specific subdirectories."""
    from collections import defaultdict
    import os
    import csv

    os.makedirs(img_directory_base, exist_ok=True)
    grouped_data = defaultdict(list)
    ...
    for node_id, entries in grouped_data.items():
        node_directory = os.path.join(img_directory_base, node_id)
        ...
        print(f"[CSV Export] Data for node {node_id} saved to {csv_filename}")
...
def fetch_sensor_data_in_chunks(spotterId, token, start_date, end_date, chunk_size_days=5):
    """
    Fetch sensor data from the Spotter API in time chunks to avoid payload limits.
    Returns a combined JSON object with all sensor messages.
    """
    combined_data = {"data": []}
    ...
    while current_start < end_datetime:
        ...
        api_url = (
            ...
            f"?spotterId={spotterId}&startDate={chunk_start}&endDate={chunk_end}"
            f"&token={token}&limit=500"
        )

        #print(f"Fetching sensor data: {chunk_start} → {chunk_end}")
        print(f"[API] Fetching sensor data for {spotterId}:\n{api_url}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Warning: Failed to fetch chunk ({response.status_code})")
        except Exception as e:
...
def main():
    ...
    #spotter_ids = ["SPOT-32071C"]  # Add as many IDs as needed
    ...
    for spotter_id in spotter_ids:
        ...
        try:
            # Fetch API data for the current Spotter ID // old method for small data sets
            #api_data = api_login(spotter_id, start_date=start_date, end_date=end_date)

            # Updated 5/23 to pagenate the data
            api_data = fetch_sensor_data_in_chunks(spotter_id, token, start_date, end_date)

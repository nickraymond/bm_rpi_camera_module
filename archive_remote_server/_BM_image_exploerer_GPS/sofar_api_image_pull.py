...
def process_grouped_data_noGPS(grouped_data, img_directory_base, csv_path, image_log_path):
...
def process_grouped_data(grouped_data, img_directory_base, csv_path, image_log_path):
    ...
    for node_id, entries in grouped_data.items():
        ...
        node_img_directory = os.path.join(img_directory_base, f"{node_id}")
        os.makedirs(node_img_directory, exist_ok=True)
        ...
        images_grouped.setdefault(node_id, [])
        ...
        for entry in entries:
            ...
            # Save each image when fully collected
            collecting_image_data = save_image(
                decoded_value, collecting_image_data, image_data, node_img_directory, image_filename,
                saved_images_count, timestamp, latitude, longitude, node_id, image_log_path,
                num_buffers_expected, buffers_received
            ...
            # If an image was just saved, capture path + lat/lon
            if "<START IMG>" in decoded_value and image_filename[0]:
                ext = os.path.splitext(image_filename[0])[1]
                formatted_timestamp = timestamp.replace(':', '-').replace('T', '_').split('.')[0]
                image_path = os.path.join("parsed_images", os.path.basename(img_directory_base), node_id,
                                          f"{formatted_timestamp}_image{ext}")
                images_grouped[node_id].append({
                    "path": image_path,
                    "lat": latitude,
                    "lon": longitude
                })
